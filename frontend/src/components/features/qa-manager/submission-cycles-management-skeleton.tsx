    ...
    <div className={UNIFIED_CARD_CLASS}>
      ...
      <div className={`${TABLE_HEAD_ROW_CLASS} px-6 py-3.5`}>
        <div className="flex gap-4">
          {[1, 2, 3, 4, 5, 6, 7].map((i) => (
            <div key={i} className={cn("h-3.5 w-14 animate-pulse rounded", SKELETON_BG_MEDIUM)} />
          ))}
        </div>
      </div>
      <div className={`divide-y ${SKELETON_DIVIDE_CLASS}`}>
        {[1, 2, 3, 4, 5].map((i) => (
          <div key={i} className="flex gap-4 px-6 py-4">
            <div className={cn("h-4 w-24 animate-pulse rounded", SKELETON_BG_SUBTLE)} />
            <div className={cn("h-4 w-20 animate-pulse rounded", SKELETON_BG_SUBTLE)} />
            <div className={cn("h-6 w-14 animate-pulse rounded-full", SKELETON_BG_SUBTLE)} />
            <div className={cn("h-4 w-28 animate-pulse rounded tabular-nums", SKELETON_BG_SUBTLE)} />
            <div className={cn("h-4 w-28 animate-pulse rounded tabular-nums", SKELETON_BG_SUBTLE)} />
            <div className={cn("h-4 w-8 animate-pulse rounded", SKELETON_BG_SUBTLE)} />
          </div>
        ))}
      </div>
    </div>
